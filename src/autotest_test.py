...
for flags_comb in flag_combinations:
    for files_comb in files_combinations:
        ...
        #print(cat_command + s21_cat_command);
        res_cat = subprocess.run(' '.join(cat_command), capture_output=True, text=True, shell=True)
        res_s21_cat = subprocess.run(' '.join(s21_cat_command), capture_output=True, text=True, shell=True)

        total_test += 1
        if res_cat.stdout == res_s21_cat.stdout:
            print(f"{' '.join(s21_cat_command)} - PASS")
            #print(f"{res_cat.stdout}\n{res_s21_cat.stdout}")
            pass_counter += 1
        else:
            print(f"{' '.join(s21_cat_command)} - FAIL")
            #print(f"{res_cat.stdout}\n{res_s21_cat.stdout}")
            #print(f"{res_cat.stderr}\n{res_s21_cat.stderr}")
            fail_counter += 1
