...
void handler(char *files[], Flags flags, bool many_files, int file_c,
  ...
  for (int i = 0; i < file_c; i++) {
    ...
    if (flags.is_only_filename) {
      print_filename(flags, p, regcomp_val, fstream, files[i]);
    } else if (flags.is_match_count) {
      print_match_c(flags, prefix, p, regcomp_val, fstream);
    } else if (flags.is_only_match) {
      print_matches(flags, prefix, p, regcomp_val, fstream);
    } else {
      print_string(flags, prefix, p, regcomp_val, fstream);
    }
...
void print_match_c(Flags flags, char prefix[], Pattern *p, int regcomp_val,
  ...
  while (fgets(buf, sizeof(buf), fstream)) {
    delete_new_line(buf);
    if (reg_handler(p, regcomp_val, buf, flags.is_invert_results)) {
      match_c++;
    }
  }
  printf("%s%d\n", prefix, match_c);
...
void print_matches(Flags flags, char prefix[], Pattern *p, int regcomp_val,
  ...
  while (fgets(buf, sizeof(buf), fstream)) {
    ...
    while (start < (int)strlen(buf)) {
      ...
      if (earliest_match.rm_so != -1 && !flags.is_invert_results) {
        printf("%s", prefix);
        if (flags.is_num_row) {
          printf("%d:", row_num);
        }
